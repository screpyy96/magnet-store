...
import { HiHome, HiViewGrid, HiCamera, HiUser, HiMenu, HiX, HiChevronDown } from 'react-icons/hi'
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 md:h-20">
            ...
            <div className="flex items-center">
              ...
              {/* Logo */}
              <Link href="/" className="ml-2 md:ml-0 flex items-center group">
            ...
            <div className="flex items-center space-x-3 md:space-x-4">
              ...
              {user ? (
                ...
                <div className="flex space-x-2">
                  <Link
                    href="/register"
                    className="hidden sm:inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-indigo-700 bg-indigo-50 hover:bg-indigo-100 transition-colors"
                  >
                    Sign up
                  </Link>
                  <Link
              ...
              {mobileNavItems.map((item) => (
                <Link
                  ...
                  className={`flex items-center px-4 py-3.5 text-base font-medium rounded-lg mx-2 transition-all duration-200 ${
                    pathname === item.href
                      ? 'bg-gradient-to-r from-indigo-50 to-indigo-100 text-indigo-700 shadow-inner'
                      : 'text-gray-700 hover:bg-gray-50 hover:pl-5'
                  } ${item.highlight ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:shadow-lg' : ''}`}
                >
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-gray-200 z-50 shadow-lg">
        <div className={`grid grid-cols-${mobileNavItems.length} h-16`}>
          {mobileNavItems.map((item) => {
